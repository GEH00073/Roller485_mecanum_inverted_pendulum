...
4台のRoller485には、それぞれ異なるI2Cアドレスを設定します。
...
アドレスの変更方法とRoller485の電源仕様は、[M5Stack公式資料](https://docs.m5stack.com/ja/guide/motor_ctl/roller485/roller485)を参照してください。Roller485は1台ずつ接続して設定し、設定後はI2Cアドレスと取付位置が分かるラベルを貼ることを推奨します。
...
本機では、市販80 mmメカナムホイールの加工、3Dプリント部品、配線部品の現物合わせを行っています。3Dプリンター、材料、メカナムホイールの寸法公差、ネジ、コネクターなどにより、加工・組立条件は変わります。そのため、すべての環境に対応する詳細な組立手順やプリント条件を提示することは困難です。

STLとSTEPデータは [`3d_models`](3d_models) に収録しています。STEPデータも参考にし、使用する部品と製作環境に合わせて調整してください。

製作上の技術的な質問は、ほかの読者も回答を参照できるよう、原則として[GitHub Issues](https://github.com/GEH00073/Roller485_mecanum_inverted_pendulum/issues/new)を利用してください。質問には、使用部品の名称と型番、問題が発生している工程、配線または組立状態が分かる写真、表示されたメッセージや症状、使用したファームウェアのバージョンまたはコミットSHAを添えてください。

更新情報は [X @GEH00073](https://x.com/GEH00073) でも案内しますが、フォローやDMは必須ではありません。
